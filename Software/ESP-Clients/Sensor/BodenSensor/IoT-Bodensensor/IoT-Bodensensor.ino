...
void initWifiManager() {
  ...
  */
  ...
  if (!wifiManager.autoConnect("IoT4School-Boden")) {
    //überflüssig nun
...
void loop() {
  ...
  if (energieModus == ECO) {
    ...
    if (abs(timestamp1 - timestamp3) < messureDuration * 1000
        && !payloadSent) {
      //falls der Pflanzenstatus kritisch ist
      if (digitalRead(BODENPIN)) {
        String payload = "{\"identifier\":\"data\",\"pflanzeKritisch\": true}";
        char payloadArray[200];
      ...
      //falls es noch okay ist
      else {
        String payload = "{\"identifier\":\"data\",\"pflanzeKritisch\": false}";
        char payloadArray[200];
