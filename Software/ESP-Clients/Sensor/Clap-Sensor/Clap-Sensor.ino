...
// KlatschSensor aktiv
int alarmModus = ON;
// Anzahl der gesamten erkannten Doppelklatscher
int clapCounter = 0;
...
long cur2 = 0;
long last1 = 0;
// Hilfsvariable zum Zählen der Klatscher
int cC = 0;
// -----------------------------------------------------
...
void onStatus(JsonObject& j) {
  ...
  String payload;
  if (alarmModus == ON) {
    payload = "{\"identifier\":\"status\",\"pwr\":\"on\", \"clapCounter\":"
        + String(clapCounter) + "}";
  } else {
    payload = "{\"identifier\":\"status\",\"pwr\":\"off\", \"clapCounter\":"
        + String(clapCounter) + "}";
  }
...
void initWifiManager() {
  ...
  //hat sich als schlecht rausgestellt
  //wifiManager.setTimeout(180);

  if (!wifiManager.autoConnect("IoT4School-Klatsch")) {
    //eigentlich überflüssig seit timeout auskommentiert ist
...
void loop() {
  ArduinoOTA.handle();
  verifyConnection();
  //lese digitalen Pin, ob Signal ausgelöst wurde
  if (digitalRead(MICRO)) {
    cur1 = millis();
    //sind seit dem letzten Signal 250ms vergangen und alarm modus aktiv?
    //250ms Zeitsperre, da der Sensor auch bei einmal klatschen mehrmals aussschlägt
    if (abs(cur1 - last1) > 250 && alarmModus == ON) {
      ...
      //falls bereits im vorigen Durchlauf geklatscht wurde
      if (cC == 1) {
        //prüfe, ob das zweite Klatschen innerhalb einer sekunde kam
        if (abs(cur1 - cur2) < 1000) {
          cC = 0;
          clapCounter++;
          String payload =
              "{\"identifier\":\"data\",\"clapDetected\":true}";
          char payloadArray[200];
          payload.toCharArray(payloadArray, 200);

          if (mqttClient.publish(finalPubTopicArray, payloadArray)) {
            Serial.println(
                "[INFO] Alarm-Payload erfolgreich versendet.");
          } else {
            Serial.println(
  ...
  //falls nach dem ersten klatschen nicht passiert ist, setze den clap counter zurück
  if (cC == 1) {
    long timeout = millis();
