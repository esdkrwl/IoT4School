...
const String modulName = "Kontakt";
...
int alarmModus = ON;
volatile int sendNotification = 0;
//Alarmzähler
long alarmCounter = 0;
// -----------------------------------------------------
...
void onConfig(JsonObject& j) {
  ...
  */
  void onStatus(JsonObject & j) {
    ...
    String payload;
    if (alarmModus == ON and digitalRead(reedPin)) {
      payload = "{\"identifier\":\"status\",\"pwr\":\"on\",\"open\": \"true\",\"alarms\":" + String(alarmCounter) + "}";
    }
    else if (alarmModus == ON and !digitalRead(reedPin)) {
      payload = "{\"identifier\":\"status\",\"pwr\":\"on\",\"open\": \"false\",\"alarms\":" + String(alarmCounter) + "}";
    }
    else if (alarmModus == OFF and digitalRead(reedPin)) {
      payload = "{\"identifier\":\"status\",\"pwr\":\"off\",\"open\": \"true\",\"alarms\":" + String(alarmCounter) + "}";
    }
    else {
      payload = "{\"identifier\":\"status\",\"pwr\":\"off\",\"open\": \"false\",\"alarms\":" + String(alarmCounter) + "}";
    }
  ...
  */
  void initWifiManager() {
    ...
    */
    ...
    if (!wifiManager.autoConnect("IoT4School-Magnetschalter")) {
      //seit timeout auskommentiert eigentlich unnötig
...
void blink() {
  ...
  */
  ...
  /*
     Wird aufgerufen, sobald sich der Zustand des Schalters ändert
  */
  void change() {
    sendNotification = 1;
  }
  ...
  void setup() {
    ...
    attachInterrupt(digitalPinToInterrupt(reedPin), change, CHANGE);
  ...
  void loop() {
    ...
    if (sendNotification and (alarmModus == ON)) {
      char payloadArray[50];
      if (digitalRead(reedPin)) {
        alarmCounter++;
        String payload = "{\"identifier\":\"data\",\"open\":true}";
        payload.toCharArray(payloadArray, 200);
      } else {
        String payload = "{\"identifier\":\"data\",\"open\":false}";   
        payload.toCharArray(payloadArray, 200);
      }
      ...
      if (mqttClient.publish(finalPubTopicArray, payloadArray)) {
        ...
        Serial.println(
          "[ERROR] Alarm-Payload konnte nicht versendet werden.");
      }
