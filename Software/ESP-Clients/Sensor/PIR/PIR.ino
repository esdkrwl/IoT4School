...
void onConfig(JsonObject& j) {
  ...
  if (j.containsKey("set_debounceTime")) {
    // prüfe Datentyp
    if (j["set_debounceTime"].is<int>()) {
      //prüfe Wertebereich
      if (j["set_debounceTime"] > 0 && j["set_debounceTime"] <= 1024) {
...
void onStatus(JsonObject& j) {
  ...
  String payload;
  if (alarmModus == ON) {
    payload = "{\"identifier\":\"status\",\"pwr\":\"on\",\"debounceTime\":"
        + String(debounceTime) + ",\"alarms\":" + String(alarmCounter)
        + "}";
  } else {
    payload =
        "{\"identifier\":\"status\",\"pwr\":\"off\",\"debounceTime\":"
            + String(debounceTime) + ",\"alarms\":"
            + String(alarmCounter) + "}";
  }
...
//Interrupt funktioniert nicht gut
/*void motionDetected(){
 currentAlarm = millis();
...
void loop() {
  ...
  verifyConnection();
  currentAlarm = millis();
  //liegt High am Pin an
  if (digitalRead(PIR_PIN)) {
    //Prüfe, ob debounce schon abgelaufen und ob Alarmmodus aktiv ist
    if (abs(lastAlarm - currentAlarm) > debounceTime * 1000
        && alarmModus == ON) {
