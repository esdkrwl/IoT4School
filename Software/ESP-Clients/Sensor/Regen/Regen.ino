...
//Schlafdauer in Zeit in Sekunden: default 15min
int deepSleepDuration = 900;
...
void initWifiManager() {
  ...
  */
  ...
  if (!wifiManager.autoConnect("IoT4School-Regen")) {
    //überflüssig nun
...
void loop() {
  ...
  //im Energiemodus falle in den deepSleep nach Ablauf der Messzeit
  if (energieModus == ECO) {
    timestamp1 = millis();
    ...
    if (abs(timestamp1 - timestamp3) < messureDuration * 1000
        && !payloadSent) {
      //falls kein Regen
      if (digitalRead(BODENPIN)) {
        String payload = "{\"identifier\":\"data\",\"rainDetected\": false}";
        char payloadArray[200];
      ...
      //falls Regen
        ...
        String payload = "{\"identifier\":\"data\",\"rainDeteced\": true}";
        char payloadArray[200];
  ...
  if (energieModus == PWR) {
    ...
    if (abs(timestamp1 - timestamp2) > 10000) {
      ...
      //falls Regen erkannt
        ...
        String payload = "{\"identifier\":\"data\",\"rainDetected\": true}";
        char payloadArray[200];
