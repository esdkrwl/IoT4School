...
int ldrValue = 0;
//D0 mit RST verbinden um DeepSleep zu aktivieren
int deepSleepSec = 60;
boolean enableDeepSleep = false;
...
void sendPayload(int i){
  String dataPayload = "{";

  dataPayload += "\"identifier\": ";
  dataPayload += "\"data\", ";

  dataPayload += "\"Helligkeit\": ";
  dataPayload += "\"" + String(i) + "\"}";

  Serial.println("[Debug] Data-Payload als JSON: " + dataPayload);

  char dataPayloadArray[dataPayload.length()+1];
  dataPayload.toCharArray(dataPayloadArray, dataPayload.length()+1);

  //Prüfe, ob neues Topic
  if(newTopicFlag){
      if (mqttClient.publish(finalPubTopicArray, dataPayloadArray)) {
        Serial.println(finalPubTopicArray);
        Serial.println("[INFO] Datenpayload erfolgreich versendet.");
        
        if(enableDeepSleep){
          Serial.println("[INFO] Goodnight Sweet Prince...");
          ESP.deepSleep(deepSleepSec * 1000000);   
        }     
      } else {
        Serial.println("[ERROR] Konnte Datenpayload nicht versenden."); 
      }
  } else {
     if (mqttClient.publish("penis", dataPayloadArray)) {
        Serial.println("[INFO] Datenpayload mittels prePubTopicArray erfolgreich versendet. ");
...
void loop() {
  ...
  sendPayload(ldrValue);
  
/*
    long now = millis();
  if (now - lastMsg > 2000 && !enableDeepSleep) {
    sendPayload(ldrValue);
