...
def on_connect(client, userdata, flags, rc):
    ...
    client.subscribe([("esp/sensor/mac/#", 2), ("esp/aktuator/mac/#", 2), ("esp/lastwill/mac/#", 2)])
...
def esp_last_will_callback(client, userdata, msg):
    ...
    c.execute('SELECT * FROM espClients')
    for row in c.fetchall():
        if last_will_mac_adr in row[1]:
...
def return_name_callback(client, userdata, msg):
    ...
    c.execute('SELECT * FROM espClients')
    for row in c.fetchall():
        logging.debug('DB Einträge: ' + str(row))
        if esp_mac_adr in row[1]:
            esp_new_name = row[4]
            ...
            logging.info("Status von " + esp_mac_adr + ": Verbunden.")
            conn.commit()

            break

    if client_found_in_db == False:
        logging.info('Mac-Adresse ' + esp_mac_adr + " bisher unbekannt.")
        c.execute('SELECT * FROM espClients')
        client_nr = 1
        for row in c.fetchall():
            if esp_name in row[4]:
               client_nr = client_nr + 1
        logging.debug('Anzahl Namen: ' + str(client_nr))
        esp_new_name = esp_name + str(client_nr)
...
#    c.close()
 #   conn.close()
    ...
    if esp_type == 'Sensor':
        ...
        client.publish(topic_str, esp_new_name, qos=2)
...
client = mqtt.Client(client_id='name_client', clean_session=True)
